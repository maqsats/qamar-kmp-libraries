...
/**
 * Wasm/web player backed by an HTMLAudioElement (via kotlinx-browser).
 *
 * Mirrors the JS actual; the only difference is that on Kotlin/Wasm the DOM
 * event-handler properties are typed `(Event) -> Unit` (not `-> dynamic`), so
 * the handler lambdas don't return a trailing `null`.
 */
